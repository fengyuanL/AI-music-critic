    ...
    def __init__(self, midibert, train_dataloader, valid_dataloader, test_dataloader, layer, 
        ...
        if model != None:    # load model
            ...
            if SeqClass and class_num != 2:
                ...
                # checkpoint = torch.load('model we just got.ckpt')
...
#            print(name, param.requires_grad)
        ...
        self.optim = AdamW(self.model.parameters(), lr=lr, weight_decay=0.02)
    ...
    def train(self):
        self.model.train()
        train_loss, train_acc, train_accl, train_accr, train_acclf, train_acclt, train_type1, train_type2 = self.reg_iteration(self.train_data, 0, self.SeqClass)
        return train_loss, train_acc, train_accl, train_accr, train_acclf, train_acclt, train_type1, train_type2

    def valid(self):
        self.model.eval()
        with torch.no_grad():
            valid_loss, valid_acc, valid_accl, valid_accr, valid_acclf, valid_acclt, val_type1, val_type2 = self.reg_iteration(self.valid_data, 1, self.SeqClass)
        return valid_loss, valid_acc, valid_accl, valid_accr, valid_acclf, valid_acclt, val_type1, val_type2

    def test(self):
        self.model.eval()
        with torch.no_grad():
            test_loss, test_acc, test_accl, test_accr, test_acclf, test_acclt, test_type1, test_type2, all_output = self.reg_iteration(self.test_data, 2, self.SeqClass)
    ...
    def iteration(self, training_data, mode, seq):
        ...
        for x, y in pbar:  # (batch, 512, 768)
            batch = x.shape[0]
            x, y = x.to(self.device, dtype = float), y.to(self.device, dtype = float)     # seq: (batch, 512, 4), (batch) / token: , (batch, 512)
    ...
    def reg_iteration(self, training_data, mode, seq):
        ...
        for count, (x, y) in enumerate(pbar):  # (batch, 512, 768)
            ...
            if not seq:
                ...
                threshold1 = 1
                threshold2 = 5
                accl = self.accuracy(y <= threshold, output.squeeze(dim=1) <= threshold) # accuracy of not acclaimed from whole sequence
                accr = self.accuracy(y > threshold, output.squeeze(dim=1) > threshold) # accuracy of acclaimed from whole sequence
                # acc = accl + accr
                acc0 = self.accuracy(y <= threshold1, output.squeeze(dim=1) <= threshold1)
                acc1 = self.accuracy(torch.logical_and(y > threshold1, y <= threshold2), torch.logical_and(output.squeeze(dim=1) > threshold1, output.squeeze(dim=1) <= threshold2))
                acc2 = self.accuracy(y > threshold2, output.squeeze(dim=1) > threshold2)
                # acc = acc0 + acc1 + acc2
                acc = accl + accr
                # accl = acc0
                # accr = acc2
                acclf = self.accuracy_local(y <= threshold, output.squeeze(dim=1) <= threshold) # local accuracy given the true result is not acclaimed
                acclt = self.accuracy_local(y > threshold, output.squeeze(dim=1) > threshold) # local accuracy given the true result is acclaimed
                #print(y)
                #print(output)
            ...
            if mode == 0:
                ...
                if (count+1) % accum_grad == 0:
                    ...
                    if local_lt_cnt == 0:
                        ...
                        local_lt_cnt = 1
                    print("Loss", round(local_loss*accum_grad/local_cnt,4), "Acc", round(local_acc.item()/local_cnt,4),
                          "Accl", round(local_accl.item()/local_cnt,4), "Accr", round(local_accr.item()/local_cnt,4),
                          "Acclf", round(torch.tensor(local_acclf).item()/local_lf_cnt,4),  "Acclt", round(torch.tensor(local_acclt).item()/local_lt_cnt,4),
                          "Type1", round(local_type1.item()/total_cnt,4), "Type2", round(local_type2.item()/local_cnt,4))
                    with open('result/finetune/acclaimed_1960_1/full_log', 'a') as outfile:
                        outfile.write("Loss: {:06f} | Acc: {:06f} | Accl: {:06f} | Accr: {:06f} | Acclf: {:06f} | Acclt: {:06f} | Type1: {:06f} | Type2: {:06f} \r".format(
                            local_loss*accum_grad/local_cnt, local_acc.item()/local_cnt, local_accl.item()/local_cnt, local_accr.item()/local_cnt,
                            local_acclf.item()/local_cnt, local_acclt.item()/local_cnt, local_type1.item()/local_cnt, local_type2.item()/local_cnt))
                    local_acc = 0
                    local_accl = 0
                    local_accr  = 0
                    local_acclf = 0
